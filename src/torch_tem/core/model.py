...
class TEMModel(nn.Module):
    ...
    def forward(self, o, locations, a_prev, state: TEMState) -> tuple[TEMOutput, TEMState]:
        ...
        p_xi = self.hpc.recall(x_, hpc_state, mode="inference") if self.hyper["use_x_cued_recall"] else None
        ...
        p_gg = self.hpc.recall(g_, hpc_state, mode="generative")
        ...
        p_gi = self.hpc.recall(g_, hpc_state, mode="generative")
    ...
    def update_memory(self, memory_prev: List[Tensor], p_inf, p_xi, p_gen_gi) -> List[Tensor]:
        ...
        M_gen = self.hpc.hebbian_updater(memory_prev[0], torch.cat(p_inf, dim=1), torch.cat(p_gen_gi, dim=1))

        # Always return 2 memories to keep HPCState invariant stable.
        if self.hyper["use_x_cued_recall"]:
            # Inference memory is identical to generative memory if using common memory,
            # and updated separately if not.
            M_inf = (
                M_gen
            ...
            # When x-cued recall is disabled, keep an inference memory slot anyway.
            # If common_memory: share the generative memory; else: keep the previous
            # inference memory unchanged.
            M_inf = M_gen if self.hyper["common_memory"] else memory_prev[1]
