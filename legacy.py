...
class Model(torch.nn.Module):
    ...
    def loss(self, g_gen, p_gen_gi, o_logits, o, g_inf, p_inf, p_xi, M_prev):
        ...
        L_x_g = utils.cross_entropy(o_logits[1], labels)
        # L_x_p: Losses generated by generative model from p_inf -> x
        L_x_p = utils.cross_entropy(o_logits[0], labels)
    ...
    def init_trainable(self):
        ...
        self.b_x = torch.nn.Parameter(torch.zeros(self.hyper["n_c"]))
            ...
            for a_i, a in enumerate(prev_iter[0].a):
                ...
                if a is None:
                    ...
                    for f, x_inf in enumerate(prev_iter[0].x_inf):
                        x_inf[a_i, :] = torch.zeros(self.hyper["n_x"][f])
    ...
    def inf_x(self, x_prev, c):
        ...
        x = [(1 - alpha[f]) * x_prev[f] + alpha[f] * c for f in range(self.hyper["n_f"])]
        return x
