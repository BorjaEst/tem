...
date = "2026-01-01"  # 2020-10-13 run 0 for successful node agent
run = "0"
index = "0"
...
environments = [
    data.World(graph, randomise_observations=True, shiny=(params["shiny"] if shiny_envs[env_i] else None)) for env_i, graph in enumerate(np.random.choice(envs, n_walks))
]
...
with torch.no_grad():
    forward = list(RolloutStream(tem, model_input))
...
figures.plot_cells(p[env_to_plot], g[env_to_plot], environments[env_to_plot], n_f_ovc=(params["n_f_ovc"] if "n_f_ovc" in params else 0), columns=25)

# Plot accuracy separated by location
plt.figure()
ax = plt.subplot(1, 2, 1)
figures.plot_map(environments[env_to_plot], np.array(to_acc[env_to_plot]), ax)
ax.set_title("Accuracy to location")
ax = plt.subplot(1, 2, 2)
figures.plot_map(environments[env_to_plot], np.array(from_acc[env_to_plot]), ax)
...
ax = figures.plot_walk(environments[env_to_plot], walks[env_to_plot], ax=ax, n_steps=max(1, int(len(walks[env_to_plot]) / 500)))
