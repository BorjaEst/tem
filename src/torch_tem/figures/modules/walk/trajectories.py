...
from torch_tem.figures.trace_access import get_location_ids_for_env, get_world, validate_env_idx


def plot(trace: TraceTree, ctx: FigureContext, *, deterministic: bool = True, seed: int = 42, max_steps: int | None = None) -> Figure:
...
def _plot_walk_deterministic(environment, location_ids: list[int], ax: plt.Axes, max_steps: int, seed: int | None = None) -> None:
