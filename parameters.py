...
def parameters():
    ...
    params["n_p"] = [g * x for g, x in zip(params["n_g_subsampled"], params["n_x"])]
    ...
    params["W_repeat"] = [torch.tensor(np.kron(np.eye(params["n_g_subsampled"][f]), np.ones((1, params["n_x"][f]))), dtype=torch.float) for f in range(params["n_f"])]
    ...
    params["W_tile"] = [torch.tensor(np.kron(np.ones((1, params["n_g_subsampled"][f])), np.eye(params["n_x"][f])), dtype=torch.float) for f in range(params["n_f"])]
    # Table for converting one-hot to two-hot compressed representation
    params["two_hot_table"] = [[0] * (params["n_o_c"] - 2) + [1] * 2]
    # We need a compressed code for each possible observation, but it's impossible to have more compressed codes than "n_o_c choose 2"
    for i in range(1, min(int(comb(params["n_o_c"], 2)), params["n_o"])):
        # Copy previous code
