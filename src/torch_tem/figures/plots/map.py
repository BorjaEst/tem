"""Axis-level map plotting primitives for TEM figures."""

from __future__ import annotations

from typing import List, Optional, Sequence

import numpy as np
from matplotlib import cm
from matplotlib.axes import Axes
from matplotlib.patches import Circle, Polygon, Rectangle


def configure_environment_axes(
    ax: Axes,
    *,
    environment: Optional[object] = None,
    radius: Optional[float] = None,
    padding_scale: float = 2.0,
) -> Axes:
    """Configure axes limits and styling for environment maps.

    Args:
        ax: Axes to configure.
        environment: Optional environment with a ``locations`` list containing
            ``{"o": x, "y": y}`` coordinates.
        radius: Optional marker radius used to pad axis limits.
        padding_scale: Multiplier applied to radius for axis padding.

    Returns:
        The configured axes.
    """
    if environment is not None and getattr(environment, "locations", None):
        coords = np.array(
            [[loc.get("o"), loc.get("y")] for loc in environment.locations],
            dtype=float,
        )
        valid = np.isfinite(coords).all(axis=1)
        coords = coords[valid]
        if coords.size > 0:
            x_min, y_min = coords.min(axis=0)
            x_max, y_max = coords.max(axis=0)
            if radius is None:
                radius = _default_radius(getattr(environment, "n_locations", 0))
            pad = (radius or 0.02) * padding_scale
            if x_min == x_max:
                x_min -= 1.0
                x_max += 1.0
            if y_min == y_max:
                y_min -= 1.0
                y_max += 1.0
            ax.set_xlim([x_min - pad, x_max + pad])
            ax.set_ylim([y_min - pad, y_max + pad])
        else:
            ax.set_xlim([0, 1])
            ax.set_ylim([0, 1])
    else:
        ax.set_xlim([0, 1])
        ax.set_ylim([0, 1])
    ax.set_aspect(1)
    ax.invert_yaxis()
    ax.axis("off")
    return ax


def action_patch(
    location_from: dict,
    location_to: dict,
    radius: float,
    color,
) -> Polygon:
    """Create a triangular patch representing a directed action.

    Args:
        location_from: Source location dict with ``o``, ``y``, and ``id`` keys.
        location_to: Destination location dict with ``o``, ``y``, and ``id`` keys.
        radius: Radius of location circles (for arrow scaling).
        color: Matplotlib color spec for the arrow.

    Returns:
        Polygon patch representing the arrow.
    """
    if location_to["id"] == location_from["id"]:
        a_dir = np.pi / 2
        xdat = location_from["o"] + radius * np.array([2 * np.cos(a_dir - np.pi / 6), 2 * np.cos(a_dir + np.pi / 6), 3 * np.cos(a_dir)])
        ydat = location_from["y"] - radius * 3 + radius * np.array([2 * np.sin(a_dir - np.pi / 6), 2 * np.sin(a_dir + np.pi / 6), 3 * np.sin(a_dir)])
    else:
        xvec = location_to["o"] - location_from["o"]
        yvec = location_from["y"] - location_to["y"]
        a_dir = np.arctan2(-yvec, xvec)
        xdat = location_from["o"] + radius * np.array([2 * np.cos(a_dir - np.pi / 6), 2 * np.cos(a_dir + np.pi / 6), 3 * np.cos(a_dir)])
        ydat = location_from["y"] + radius * np.array([2 * np.sin(a_dir - np.pi / 6), 2 * np.sin(a_dir + np.pi / 6), 3 * np.sin(a_dir)])
    return Polygon(np.stack([xdat, ydat], axis=1), color=color)


def plot_map(
    ax: Axes,
    environment: object,
    values: np.ndarray,
    *,
    min_val: Optional[float] = None,
    max_val: Optional[float] = None,
    num_cols: int = 100,
    location_cm: str = "viridis",
    action_cm: str = "Pastel1",
    do_plot_actions: bool = False,
    shape: str = "circle",
    radius: Optional[float] = None,
) -> Axes:
    """Render an environment map with per-location scalar values.

    Args:
        ax: Axes to draw on.
        environment: Environment object with ``locations`` and ``n_locations``.
        values: Per-location scalar values (shape: ``(n_locations,)``).
        min_val: Minimum value for colormap normalization (None = auto).
        max_val: Maximum value for colormap normalization (None = auto).
        num_cols: Number of discrete colors in the colormap.
        location_cm: Colormap name for location coloring.
        action_cm: Colormap name for action arrows.
        do_plot_actions: Whether to draw action transition arrows.
        shape: Location marker shape ("circle" or "square").
        radius: Radius/half-width of location markers (None = auto-scale).

    Returns:
        The axes with the environment map rendered.
    """
    values = np.asarray(values, dtype=float)
    has_finite = values.size > 0 and np.isfinite(values).any()

    min_val = (np.nanmin(values) if has_finite else 0.0) if min_val is None else min_val
    max_val = (np.nanmax(values) if has_finite else 1.0) if max_val is None else max_val

    location_cm = cm.get_cmap(location_cm, num_cols)
    action_cm = cm.get_cmap(action_cm, max(getattr(environment, "n_actions", 0), 1))

    if values.size == 0:
        plotvals = np.zeros(values.shape)
        nan_mask = np.zeros(values.shape, dtype=bool)
    else:
        if max_val != min_val:
            plotvals = np.floor((values - min_val) / (max_val - min_val) * num_cols)
        else:
            plotvals = np.zeros(values.shape)
        nan_mask = np.isnan(values)
        plotvals = np.where(nan_mask, 0, plotvals)

    if radius is None:
        radius = _default_radius(getattr(environment, "n_locations", 0))

    configure_environment_axes(ax, environment=environment, radius=radius)

    location_patches: List = []
    action_patches: List = []

    for i, location in enumerate(environment.locations):
        is_nan = nan_mask[i] if nan_mask.size else False
        if is_nan:
            color = "#d9d9d9"
            edgecolor = "#444444"
            alpha = 1.0
        else:
            color = location_cm(int(plotvals[i]))
            edgecolor = None
            alpha = 1.0

        if shape == "square":
            patch = Rectangle(
                (location["o"] - radius / 2, location["y"] - radius / 2),
                radius,
                radius,
                color=color,
                alpha=alpha,
                edgecolor=edgecolor,
                linewidth=0.6 if edgecolor else 0.0,
            )
        else:
            patch = Circle(
                (location["o"], location["y"]),
                radius,
                color=color,
                alpha=alpha,
                edgecolor=edgecolor,
                linewidth=0.6 if edgecolor else 0.0,
            )
        location_patches.append(patch)

        if do_plot_actions:
            for action in location["actions"]:
                if action["probability"] > 0:
                    locations_to = [environment.locations[loc_to] for loc_to in np.where(np.array(action["transition"]) > 0)[0]]
                    for loc_to in locations_to:
                        action_patches.append(action_patch(location, loc_to, radius, action_cm(action["id"])))

    for location in environment.locations:
        if location.get("shiny", False):
            if shape == "square":
                outline = Rectangle(
                    (location["o"] - radius / 2, location["y"] - radius / 2),
                    radius,
                    radius,
                    linewidth=1,
                    facecolor="none",
                    edgecolor=[1, 0, 0],
                )
            else:
                outline = Circle(
                    (location["o"], location["y"]),
                    radius,
                    linewidth=1,
                    facecolor="none",
                    edgecolor=[1, 0, 0],
                )
            location_patches.append(outline)

    for patch in location_patches + action_patches:
        ax.add_patch(patch)
    return ax


def plot_walk(
    ax: Axes,
    environment: object,
    walk: Sequence,
    *,
    max_steps: Optional[int] = None,
    n_steps: int = 1,
) -> Axes:
    """Overlay a walk trajectory onto an environment map.

    Args:
        ax: Axes to draw on.
        environment: Environment object with ``locations``.
        walk: Walk trajectory as a sequence of (location, observation, action).
        max_steps: Maximum number of steps to plot (None = plot all).
        n_steps: Plot every ``n_steps``-th step (1 = plot all).

    Returns:
        The axes with the walk overlay rendered.
    """
    if not walk:
        ax.text(0.5, 0.5, "No walk", ha="center", va="center", fontsize=10)
        ax.axis("off")
        return ax

    max_steps = len(walk) if max_steps is None else min(max_steps, len(walk))

    location_patches = [patch for patch in ax.patches if isinstance(patch, (Circle, Rectangle))]
    if location_patches:
        last_patch = location_patches[-1]
        if isinstance(last_patch, Circle):
            radius = last_patch.get_radius()
        else:
            radius = last_patch.get_width()
    else:
        radius = 0.02

    prev_loc = np.array(
        [
            environment.locations[walk[0][0]["id"]]["o"],
            environment.locations[walk[0][0]["id"]]["y"],
        ]
    )
    for step_i in range(1, max_steps, n_steps):
        new_loc = np.array(
            [
                environment.locations[walk[step_i][0]["id"]]["o"],
                environment.locations[walk[step_i][0]["id"]]["y"],
            ]
        )
        new_loc = new_loc + 0.8 * (-radius + 2 * radius * np.random.rand(*new_loc.shape))
        color_intensity = step_i / max_steps
        ax.plot(
            [prev_loc[0], new_loc[0]],
            [prev_loc[1], new_loc[1]],
            color=[color_intensity] * 3,
        )
        prev_loc = new_loc
    return ax


def plot_actions(
    ax: Axes,
    environment: object,
    *,
    field: str = "probability",
    min_val: Optional[float] = None,
    max_val: Optional[float] = None,
    num_cols: int = 100,
    action_cm: str = "viridis",
) -> Axes:
    """Visualize action properties across the environment.

    Args:
        ax: Axes to draw on.
        environment: Environment object with ``locations``.
        field: Action property to visualize (e.g., "probability").
        min_val: Minimum value for colormap normalization (None = auto).
        max_val: Maximum value for colormap normalization (None = auto).
        num_cols: Number of discrete colors in the colormap.
        action_cm: Colormap name for action coloring.

    Returns:
        The axes with action visualization rendered.
    """
    if not getattr(environment, "locations", None):
        configure_environment_axes(ax, environment=environment)
        return ax

    if min_val is None:
        min_val = min(action[field] for location in environment.locations for action in location["actions"])
    if max_val is None:
        max_val = max(action[field] for location in environment.locations for action in location["actions"])

    action_cm = cm.get_cmap(action_cm, num_cols)
    radius = _default_radius(getattr(environment, "n_locations", 0))

    configure_environment_axes(ax, environment=environment, radius=radius)

    location_patches: List = []
    action_patches: List = []

    for location in environment.locations:
        location_patches.append(Circle((location["o"], location["y"]), radius, color=[0, 0, 0]))
        for action in location["actions"]:
            if action["probability"] > 0:
                if max_val != min_val:
                    color_idx = int(np.floor((action[field] - min_val) / (max_val - min_val) * num_cols))
                else:
                    color_idx = 0
                action_color = action_cm(color_idx)
                locations_to = [environment.locations[loc_to] for loc_to in np.where(np.array(action["transition"]) > 0)[0]]
                for loc_to in locations_to:
                    action_patches.append(action_patch(location, loc_to, radius, action_color))

    for patch in location_patches + action_patches:
        ax.add_patch(patch)
    return ax


def _default_radius(n_locations: int) -> float:
    if n_locations <= 0:
        return 0.05
    return 2 * (0.01 + 1 / (10 * np.sqrt(n_locations)))
