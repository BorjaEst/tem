...
def legacy_to_typed(params: Dict[str, Any]) -> ConfigWrapper:
    ...
    # Wrap in ConfigWrapper for TEMModel
    return ConfigWrapper(architecture=architecture, inference=inference, training=training, environment=environment)
...
def _create_merged_architecture(params: Dict[str, Any]):
    ...
    class MergedArchConfig:
        def __init__(self, base_arch, params_dict):
            ...
            # Add environment fields needed by TransitionModel
            self.n_actions = params_dict["n_actions"]
            self.has_static_action = params_dict["has_static_action"]
