...
class MECModel(nn.Module):
    ...
    @property
    def grid_n_freq(self) -> List[int]:
        """Return grid-cell counts per frequency module (alias for `shape`)."""
        return self.shape[: self.ovc.start] if self.ovc.n_freq > 0 else self.shape
