...
def performance(forward, model, environments):
    ...
    for env_i, env in enumerate(environments):
        ...
        for step in forward:
            ...
            correct.append((torch.argmax(step.x_gen[2][env_i]) == torch.argmax(step.o[env_i])).numpy())
...
def location_accuracy(forward, model, environments):
    ...
    for env_i, env in enumerate(environments):
        ...
        for step_i, step in enumerate(forward[1:]):
            ...
            correct_to[step.g[env_i]["id"]].append((torch.argmax(step.x_gen[2][env_i]) == torch.argmax(step.o[env_i])).numpy().tolist())
            ...
            correct_from[forward[step_i].g[env_i]["id"]].append((torch.argmax(step.x_gen[2][env_i]) == torch.argmax(step.o[env_i])).numpy().tolist())
        # Add performance and visitation fractions of this environment to performance list across environments
    ...
    for env_i, env in enumerate(environments):
        ...
        for step in forward[1:]:
            ...
            if location_visited[step.g[env_i]["id"]] and prev_a is not None and not action_taken[prev_g, prev_a]:
                ...
                correct_zero_shot.append((torch.argmax(step.x_gen[2][env_i]) == torch.argmax(step.o[env_i])).numpy())
...
def compare_to_agents(forward, model, environments, include_stay_still=True):
    ...
    for env_i, env in enumerate(environments):
        ...
        for step in forward[1:]:
            ...
            correct_model.append((torch.argmax(step.x_gen[2][env_i]) == torch.argmax(step.o[env_i])).numpy())
            # Update node agent prediction for this step: correct when this state was visited beofre, otherwise chance
            correct_node.append(True if location_visited[step.g[env_i]["id"]] else np.random.randint(model.hyper["n_o"]) == torch.argmax(step.o[env_i]).numpy())
            ...
            correct_edge.append(True if prev_a is None else True if action_taken[prev_g, prev_a] else np.random.randint(model.hyper["n_o"]) == torch.argmax(step.o[env_i]).numpy())
            # Update the previous action as taken
            if prev_a is not None:
                action_taken[prev_g, prev_a] = True
            # And update the previous iteration to the current iteration
...
def rate_map(forward, model, environments):
    # Store location o cell firing rate matrix for abstract and grounded location representation across environments
    all_g, all_p = [], []
    ...
    for env_i, env in enumerate(environments):
        ...
        for cells, n_cells in zip([p, g], [model.hyper["n_p"], model.hyper["n_g"]]):
            for f, frequency in enumerate(cells):
                ...
                for l, location in enumerate(frequency):
                    frequency[l] = (
                        sum(location[int(len(location) / 2) :]) / len(location[int(len(location) / 2) :]) if len(location[int(len(location) / 2) :]) > 0 else np.zeros(n_cells[f])
                    )
                # Then concatenate the locations to get a [locations o cells for this frequency] matrix
        ...
        all_g.append(g)
        all_p.append(p)
    # Return list of locations o cells matrix of firing rates for each frequency module for each environment
    return all_g, all_p
