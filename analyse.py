...
def compare_to_agents(forward, model, environments, include_stay_still=True):
    ...
    for env_i, env in enumerate(environments):
        ...
        for step in forward[1:]:
            ...
            correct_node.append(True if location_visited[step.g[env_i]["id"]] else np.random.randint(model.hyper["n_o"]) == torch.argmax(step.x[env_i]).numpy())
            ...
            correct_edge.append(True if prev_a is None else True if action_taken[prev_g, prev_a] else np.random.randint(model.hyper["n_o"]) == torch.argmax(step.x[env_i]).numpy())
            # Update the previous action as taken
            if prev_a is not None:
                action_taken[prev_g, prev_a] = True
            # And update the previous iteration to the current iteration
