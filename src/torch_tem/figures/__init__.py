...
def plot_map(
    environment, values, ax=None, min_val=None, max_val=None, num_cols=100, location_cm="viridis", action_cm="Pastel1", do_plot_actions=False, shape="circle", radius=None
):
    ...
    for i, location in enumerate(environment.locations):
        ...
        location_patches.append(
            plt.Rectangle((location["o"] - radius / 2, location["y"] - radius / 2), radius, radius, color=location_cm(int(plotvals[i])))
            if shape == "square"
            else plt.Circle((location["o"], location["y"]), radius, color=location_cm(int(plotvals[i])))
        )
    ...
    for location in environment.locations:
        ...
        if location["shiny"]:
            ...
            location_patches.append(
                plt.Rectangle((location["o"] - radius / 2, location["y"] - radius / 2), radius, radius, linewidth=1, facecolor="none", edgecolor=[1, 0, 0])
                if shape == "square"
                else plt.Circle((location["o"], location["y"]), radius, linewidth=1, facecolor="none", edgecolor=[1, 0, 0])
            )
    # Add patches to axes
...
def plot_actions(environment, field="probability", ax=None, min_val=None, max_val=None, num_cols=100, action_cm="viridis"):
    ...
    for i, location in enumerate(environment.locations):
        ...
        location_patches.append(plt.Circle((location["o"], location["y"]), radius, color=[0, 0, 0]))
...
def plot_walk(environment, walk, max_steps=None, n_steps=1, ax=None):
    ...
    radius = (
        (ax.patches[location_patches[-1]].get_radius() if type(ax.patches[location_patches[-1]]) is plt.Circle else ax.patches[location_patches[-1]].get_width())
        if len(location_patches) > 0
        else 0.02
    )
    # Initialise previous location: location of first location
    prev_loc = np.array([environment.locations[walk[0][0]["id"]]["o"], environment.locations[walk[0][0]["id"]]["y"]])
    ...
    for step_i in range(1, max_steps, n_steps):
        ...
        new_loc = np.array([environment.locations[walk[step_i][0]["id"]]["o"], environment.locations[walk[step_i][0]["id"]]["y"]])
    ...
    for cells, names in zip([p, g], ["Hippocampal", "Entorhinal"]):
        ...
        for f, loc_rates in enumerate(cells):
            ...
            ax[n_rows_f[f], int(columns / 2)].set_title(
                names + ("" if f < len(cells) - n_f_ovc else " object vector ") + " cells, frequency " + str(f if f < len(cells) - n_f_ovc else f - (len(cells) - n_f_ovc))
            )
