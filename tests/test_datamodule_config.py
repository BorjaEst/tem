...
def test_removed_policy_walk_length_fields_are_rejected():
    """Legacy policy walk-length curriculum fields were removed and must fail."""
    with pytest.raises(Exception):
        RandomPolicyConfig(walk_length_min=25)

    with pytest.raises(Exception):
        DistancePolicyConfig(walk_length_max=100)

    with pytest.raises(Exception):
        QLearningPolicyConfig(walk_length_curriculum=True)
...
def test_removed_shiny_navigation_fields_are_rejected():
    """Legacy shiny navigation fields were removed and must fail."""
    with pytest.raises(Exception):
        ShinyPolicyConfig(navigation_policy="distance")

    with pytest.raises(Exception):
        ShinyPolicyConfig(gamma=0.7)
